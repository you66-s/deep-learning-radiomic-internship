...
def plot_loss_curves(results, run_name):
    ...
    plt.legend()
    plt.grid(True)
    plot_path = f"artifacts/loss_curves/{run_name}_loss_curve.png"
    plt.savefig(plot_path)
    return plt.gcf()
...
def glrm_hybrid_scaler(dataset, power_features, quantile_features, is_train=True, preprocessors=None):
    ...
    missing_cols = [c for c in all_cols if c not in dataset.columns]
    if missing_cols:
        raise ValueError(f"Missing columns in dataset: {missing_cols}")
    
    if not is_train and preprocessors is None:
        raise ValueError("Preprocessors must be provided for inference")
    ...
    if is_train:
        ...
        medians = preprocessors["medians"]
    df.loc[:, all_cols] = df[all_cols].fillna(medians)    
    ...
    if is_train:
        ...
        if len(power_features) > 0:
            ...
            scaler_power = RobustScaler()
            power_data = pt_yeo.fit_transform(df[power_features])
            df.loc[:, power_features] = scaler_power.fit_transform(power_data)
            preprocessors["pt_yeo"] = pt_yeo
            preprocessors["scaler_power"] = scaler_power
        ...
        if len(quantile_features) > 0:
            ...
            scaler_quantile = RobustScaler()
            quantile_data = qt.fit_transform(df[quantile_features])
            df.loc[:, quantile_features] = scaler_quantile.fit_transform(quantile_data)
            preprocessors["qt"] = qt
            preprocessors["scaler_quantile"] = scaler_quantile
    ...
    # --- Inference path ---
        ...
        if len(power_features) > 0:
            power_data = preprocessors["pt_yeo"].transform(df[power_features])
            df.loc[:, power_features] = preprocessors["scaler_power"].transform(power_data)
        
        if len(quantile_features) > 0:
            quantile_data = preprocessors["qt"].transform(df[quantile_features])
            df.loc[:, quantile_features] = preprocessors["scaler_quantile"].transform(quantile_data)
