...
from training_engine import evaluate_and_plot, train_model, plot_loss_curves, glrm_hybrid_scaler
import numpy as np
from helpers import check_tensor_integrity

# Hyperparameters
tensor_path   = "data/processed_tensors/GLRM/224x224_scaled"
LR = 1e-4
WEIGHT_DECAY = 1e-2
EPOCHS = 25
TRAIN_BATCH_SIZE = 64
...
CONFIG = {
        ...
        "dataset": "glrm_cleaned_texture_dataset",
        "epochs": EPOCHS,
...
target_cols = [c for c in dataset.columns if c.startswith("rlm_")]
...
train_df, preprocessors = glrm_hybrid_scaler(dataset=train_df, power_features=POWER_FEATURES, quantile_features=QUANTILE_FEATURES, is_train=True)
val_df, _               = glrm_hybrid_scaler(dataset=val_df, power_features=POWER_FEATURES, quantile_features=QUANTILE_FEATURES, is_train=False, preprocessors=preprocessors)
test_df, _              = glrm_hybrid_scaler(dataset=test_df, power_features=POWER_FEATURES, quantile_features=QUANTILE_FEATURES, is_train=False, preprocessors=preprocessors)
...
num_radiomic_features = len([c for c in dataset.columns if c.startswith("rlm_")])
...
model_path = f"artifacts/saved-models/glcm-features/{RUN_NAME}.pth"
torch.save(model.state_dict(), model_path)

test_results, fig_eval = evaluate_and_plot(model, test_loader, target_cols, device, RUN_NAME)
fig_loss = plot_loss_curves(history, run_name=RUN_NAME)

wandb.log({
    "loss_curve": wandb.Image(fig_loss),
